...
LRESULT WINAPI  RegisterTraceBlock(void *pTrace, int sizeTrace);
...
HWND WINAPI	ListOfAllWindows( void );
HWND WINAPI	FindTopMostWindows( void );
BOOL WINAPI	SetWindowTopMost( HWND hwnd, BOOL bTop, BOOL bVisitSiblings );
...
typedef struct timer {			/* private timer structure*/
	...
	int	freeReq;	/* free was requested */
	struct timer *pNext;	/* order by dwClockExpires*/
} TIMER, *PTIMER;

PTIMER WINAPI	GetTimerList(void);

BOOL WINAPI	RemoveAllTimers(HWND hwnd, UINT id);
