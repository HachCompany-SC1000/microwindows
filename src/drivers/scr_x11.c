...
#define USE_SCREENVALUES_FROM_SCFB  (1) 
#if USE_SCREENVALUES_FROM_SCFB		
#warning USE_SCREENVALUES_FROM_SCFB 
// 1. approach: we take the same values as the sc1000 display  ==> 14/5
// 2. approach: we measure our display width: 320px/111mm, height: 240px/83mm  ==> 3/3
// 3. approach: we use values from X11 functions  ==> 29/29
// 4. approach: we use some fancy values  ==> 1/33
	psi->xdpcm =  3; //29; //14;         
	psi->ydpcm =  3; //29; //5;
	DPRINTF("X11_getscreeninfo() - using x(y)dpcm = [%d/%d] for [%d/%d]\n", psi->xdpcm, psi->ydpcm,  
		    (DisplayWidth(x11_dpy, x11_scr) * 10) / DisplayWidthMM(x11_dpy, x11_scr),
			(DisplayHeight(x11_dpy, x11_scr) * 10) / DisplayHeightMM(x11_dpy, x11_scr) 
		   );
#else
