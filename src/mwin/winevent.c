...
extern BOOL bTopMostWindowExists;

#if ( (DOS_TURBOC | DOS_QUICKC | _MINIX | VXWORKS) != 0 )
static int
abs(int n)
{
	return n >= 0? n: -n;
...
MwDeliverMouseEvent(int buttons, int changebuttons, MWKEYMOD modifiers)
{
	...
	// catch away all "button downs" for non-WS_EX_TOPMOST windows:
	if ( bTopMostWindowExists && (!( hwnd->exstyle & WS_EX_TOPMOST )) &&
	     (buttons &(MWBUTTON_L|MWBUTTON_M|MWBUTTON_R)) ) {
		HWND hwndTop = FindTopMostWindows();
		//printf("MwDeliverMouseEvent() - CAUGHT\n");
		if (hwndTop) 
			MwRaiseWindow( hwndTop );
		return;  // and ignore
	}
