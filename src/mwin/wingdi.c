 ...
 * Rainer 22.8.03
 *	MwDrawText() corected for right alligned output
 */
...
/* unicode utf-8*/
int WINAPI
DrawTextU8(HDC hdc, LPCSTR lpString, int nCount, LPRECT lpRect, UINT uFormat)
{
#if USE_COLORED_BACKGROUND
#warning USE_COLORED_BACKGROUND in DrawTextU8 
	...
	if (! (uFormat & DT_CALCRECT)) {
		...
		MwDrawText(hdc, lpString, nCount, &r, uFormat | DT_CALCRECT, MWTF_UTF8);
		//FastFillRect(hdc, &r, col);
		FastFillRect(hdc, lpRect, col);
	}
#endif
	return MwDrawText(hdc, lpString, nCount, lpRect, uFormat, MWTF_UTF8);
}
	...
	int flags)
{
	...
	if(uFormat & DT_CALCRECT) {
		if(uFormat & DT_RIGHT) {
			...
			lpRect->left = (lpRect->left + lpRect->right - width) / 2;
	...
	if(uFormat & DT_CENTER)
		...
		x = lpRect->right - width;	// 22.8.03 Rainer
...
/* Microwindows only*/
BOOL WINAPI
DrawFileImage(HDC hdc, int x, int y, int h, int w, char *file)
{
	HWND		hwnd;
	POINT		pt;

	STATUS_TRACE(808002000);
	hwnd = MwPrepareDC(hdc);
	STATUS_TRACE(808002001);
	STATUS_TRACE(hwnd);
	if(!hwnd || !file)
		return FALSE;
	pt.x = x;
	pt.y = y;
	if(MwIsClientDC(hdc)) {
		STATUS_TRACE(808002002);
		ClientToScreen(hwnd, &pt);
		STATUS_TRACE(808002003);
	}

	GdDrawImageFromFile(hdc->psd, pt.x, pt.y, h, w, file, 0x0 );
	STATUS_TRACE(808002004);
	return TRUE;
}
...
GetStockObject(int nObject)
{
	...
	if(nObject >= 0 && nObject < MAXSTOCKOBJECTS) {
		...
		if(pObj->hdr.type == OBJ_FONT) {
			...
			if(pFont->pfont == NULL) {
				pFont->pfont = GdCreateFont(&scrdev, "default", 16 /*15*/, NULL);
				               //GdCreateFont(&scrdev, pFont->name, 0, NULL);
				if (!pFont->pfont)
