 ...
 * Modified to output 16 bit R5G6B5 format
 */
...
char	oname[MAX_FILENAME_LEN];
   ...
   else if(argc && strcmp(*argv, "-16") == 0) { /* check for -16 flag */
	use16_flag = -1;
	argc--;
	argv++;
   }
   ...
   if((argc == 0) || (o_flag && (argc != 2))) {
	fprintf(stderr, "Usage: convbmp [-s|-16] <bmpfile>...\n"
	                "   OR: convbmp [-s|-16] -o <outfile> <bmpfile>\n"
	                "    -s : use ACK format for Minix\n"
	                "    -16: use RGB 565 16 bit format\n"
	);
	return(-1);
   }
...
UCHAR *p = (UCHAR *)&l;
   ...
   if((linebuffer = (UCHAR *)malloc(linesize)) == (UCHAR *)NULL) {
   	fprintf(stderr, "Error with malloc(%d)\n", linesize);
	return(-1);
   }

   if(use16_flag) {
	printf("/* Generated by convbmp-logo*/\n");
	printf("/****************************/\n");
   ...
   if(use16_flag) {
	printf("unsigned short imagebits[] = {\n");
   }
   else if(!s_flag) {
	...
	default:
		for(n=0; n<linesize; ++n) {
			if(use16_flag) {
				...
				g = linebuffer[n]; ++n;
				b = linebuffer[n];
				v16 = (((USHORT)(b>>3))<<11) + (((USHORT)(g>>2))<<5)  + (((USHORT)(r>>3))<<0);

				if ( (!v16) && ((r&7)+(g&3)+(b&7) >3 ) ) {
					v16 = (((USHORT)1)<<11) + (((USHORT)1)<<5)  + (((USHORT)1)<<0);
				}
				printf("%u,", (UINT)v16);
			}
			else if(!s_flag)
