 ...
 *
 * <jochen.sparbier@drlange.de> [2003/09/25]:
 *   - SetTimer, KillTimer extended to handle multiple timers.
 *   - Timer 'startTicks' are renewed, when timeout for new event
 *     would cause an overrun.
 */
...
BOOL bTopMostWindowExists = FALSE;      /* flag: there exists at least one window with extended style  */


LRESULT WINAPI
RegisterTraceBlock( void *pTrace, int sizeTrace )
{
	g_psStatTrace = (StatusTrace_t*)pTrace;
	g_lsStatTraceMaxIdx = sizeTrace-1;
	return TRUE;
}
	...
	HWND hwndParent, HMENU hMenu, HINSTANCE hInstance, LPVOID lpParam)
{
	...
	if(dwStyle & WS_CHILD) {
		hwndOwner = NULL;
		if (hwndParent && (hwndParent->exstyle & WS_EX_TOPMOST) )
	...
	if ( dwExStyle & WS_EX_TOPMOST ) {
		//printf("CreateWindowEx() - WS_EX_TOPMOST (%s)\n", (lpClassName ? lpClassName : "noname") );
		bTopMostWindowExists = TRUE;
	}
	return wp;
}
...
HWND WINAPI
ListOfAllWindows( void )
{
	return listwp;
}

HWND WINAPI
FindTopMostWindows( void )
{
	HWND wp = listwp;
	while (wp) {
		if (wp->exstyle & WS_EX_TOPMOST ) {
			while ( wp->parent && (wp->parent->exstyle & WS_EX_TOPMOST) )
				wp = wp->parent;
			//printf("FindTopMostWindows() - found (%s)\n", wp->pClass->lpszClassName );
			return wp;
		}
		wp = wp->next;
	}
	//printf("FindTopMostWindows() - NOT found\n" );
	return (HWND)NULL;
}

BOOL WINAPI
SetWindowTopMost( HWND hwnd, BOOL bTop, BOOL bVisitSiblings )
{
	static  BOOL bsFindTopMostWindow = TRUE;
	BOOL    bFindTopMostWindow = bsFindTopMostWindow;

	bsFindTopMostWindow = FALSE; /* test only at function exit */
	...
	do {
		if (bTop) 
			hwnd->exstyle |= WS_EX_TOPMOST;
		else
			hwnd->exstyle &= (~WS_EX_TOPMOST);

		//printf("SetWindowTopMost() - %s hwnd=0x%08x\n", bTop?"SET":"CLR", (unsigned int)hwnd ));
		
		if (hwnd->children)
			SetWindowTopMost(hwnd->children, bTop, TRUE);

		hwnd = hwnd->siblings;
	} while (bVisitSiblings && hwnd);

	if (bFindTopMostWindow) {
		bsFindTopMostWindow = TRUE;  /* restore initial setting */
		bTopMostWindowExists = (BOOL)FindTopMostWindows();
	}
...
MwDestroyWindow(HWND hwnd,BOOL bSendMsg)
{
	...
	/*
	 * Kill all timers for the window.
	 */
	RemoveAllTimers( wp, 0 );
...
static PTIMER timerList = NULL;	/* now: timerList FIXING ...*/
static PTIMER timerAct = NULL;  /* during timer service the serviced timer is not in timerList but here */



#if 0
static void
ListAllTimers(PTIMER pT)
{
static int cnt=0;
printf("TIMER ListAllTimers  (cycle=%d)~~ GetTickCount=%u\n", ++cnt, (unsigned int)GetTickCount() );
	while (pT) {
printf("          + this=0x%08x hwnd=0x%08x id=0x%08x to=%u exp=%u next=0x%08x\n",
	(unsigned int)pT, (unsigned int)pT->hwnd, (unsigned int)pT->idTimer, (unsigned int)pT->uTimeout, (unsigned int)pT->dwClockExpires, (unsigned int)pT->pNext );
		pT = pT->pNext;
	}
fflush(stdout);
}
...
InsertTimer(PTIMER pT)
{
	...
	if (pT->dwClockExpires < timerList->dwClockExpires) {
		pT->pNext = timerList;
		timerList = pT;
		return TRUE;
	}
	pM = pN = timerList;
	while (pN && pT->dwClockExpires >= pN->dwClockExpires) {
		pM = pN;
		pN = pN->pNext;
	}
	pM->pNext = pT;
...
static void FreeTimer(PTIMER pT)
{
	free(pT);
}
...
BOOL WINAPI
RemoveAllTimers(HWND hwnd, UINT id)
{
#define COND(pT) (hwnd==pT->hwnd && ((id && id==pT->idTimer) || !id))
	int ret = FALSE;
	PTIMER pM, pN ,pL;

	if (timerAct && COND(timerAct))
		timerAct->freeReq = TRUE;

	while (timerList && COND(timerList)) {
		pM = timerList;
		timerList = pM->pNext;
		FreeTimer(pM);
		ret = TRUE;
...
static void
RescaleTimerList(PTIMER list, DWORD inc)
{
	while (list) {
		list->dwClockExpires -= inc;
		list = list->pNext;
	}
	return;
}
...
static void
SetExpire( PTIMER pT )
{  /* *pT MUST NOT be in timerList */
	DWORD  dwTime = GetTickCount();
	DWORD  incTicks = 0;   /* used to adjust the timeout of all events in timerList */
	DWORD  tmoTicks = dwTime + pT->uTimeout;
	if ( tmoTicks<dwTime || tmoTicks<pT->uTimeout ) {
		/* overrun: we need to adjust startTicks & all waiting events: */
		startTicks = 0;
		startTicks = GetTickCount();
		incTicks = dwTime - GetTickCount();
		RescaleTimerList( timerList, incTicks );
	}

	pT->dwClockExpires = dwTime + pT->uTimeout - incTicks;
}

UINT WINAPI
SetTimer(HWND hwnd, UINT idTimer, UINT uTimeout, TIMERPROC lpTimerFunc)
{
	PTIMER pT = (PTIMER)calloc(sizeof(TIMER),1);
	pT->hwnd = hwnd;
	if (idTimer) {
		KillTimer(hwnd, idTimer);
		pT->idTimer = idTimer;
	}
	else	/* assign timer id based on struct timer memory address - unique! */
		pT->idTimer = (UINT)pT;
	pT->uTimeout = uTimeout;
	pT->lpTimerFunc = lpTimerFunc;
	SetExpire(pT);
	InsertTimer(pT);
	return pT->idTimer;
}
...
{
	return RemoveAllTimers(hwnd, idTimer);
}
...
MwGetNextTimeoutValue(void)
{
	...
	if(pT) {
		timeout = pT->dwClockExpires - GetTickCount();
		if(timeout > 0) {
			return timeout;
		}
	}
...
MwHandleTimers(void)
{
	...
	pT = timerList; pL = timerList;
	while (pT) {
		/* determine if timer expired*/
		if (pT->dwClockExpires <= dwTime) {  /* serve this event: */
			pN = pT->pNext;
			/* remove pT from list: */
			if (pT==timerList)
				timerList = pN;
			...
			timerAct = pT;
			if (pT->lpTimerFunc)
				pT->lpTimerFunc(pT->hwnd, WM_TIMER, pT->idTimer, dwTime);
			else
				PostMessage(pT->hwnd, WM_TIMER, pT->idTimer, 0);
			timerAct = NULL;
			...
			break; /* handle only one event in MwHandleTimers() */
		}
		else {
			pL = pT;
			pT = pT->pNext;
