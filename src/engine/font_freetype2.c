 ...
 */
#define FREETYPE2_CACHE_FONT_SIZE_LIMIT 150
			      ...
			      char *filename, MWCOORD height)
{
	...
	memset(&scaler,0,sizeof(FTC_ScalerRec));
 ...
 */
int pixel_width_hook_is = 0, pixel_width_hook_shall = 0;
...
freetype2_setfontsize(PMWFONT pfont, MWCOORD fontsize)
{
	...
	if (pixel_width_hook_is!=0 && pixel_width_hook_shall!=0) {
		pixel_width = fontsize * pixel_width_hook_shall / pixel_width_hook_is;
		pixel_width_hook_is = 0;
		pixel_width_hook_shall = 0;
	}
	else {
...
freetype2_setfontrotation(PMWFONT pfont, int tenthdegrees)
{
	...
	if (pf->fontrotation == 0) {
		...
		pf->matrix.yy = sincosvec.x;
		pf->matrix.xx = sincosvec.x;
		pf->matrix.yx = sincosvec.y;
		pf->matrix.xy = -sincosvec.y;
	}
}
                        ...
                        int *pdescent)
{
	...
	err = freetype2_get_glyph_size(pf, face, glyph, padvance, pascent, pdescent);
	if (ch >= 0xe31 && ch < 0xe4e) {  /* check for Thai sub/super scripts */
		if (!( ch==0xe32 ||
		       ch==0xe33 ||
		      (ch>=0xe3b && ch<=0xe46) )) {
			*padvance = 0;
...
static void scale_glyph( FT_Glyph outline_glyph)
{
#define MY_SCALE 2
	FT_OutlineGlyph  glyph   = (FT_OutlineGlyph)outline_glyph;
	int i, n, s;
	n = glyph->outline.n_points;

	outline_glyph->advance.x = outline_glyph->advance.x / MY_SCALE;
	for (i=0; i<n; i++) {
		//printf( "\t (x y) = (%4d  %4d) -> ", (glyph->outline.points+i)->x, (glyph->outline.points+i)->y );
		s = (glyph->outline.points+i)->x < 0 ? 1 : -1;
		(glyph->outline.points+i)->x = (((glyph->outline.points+i)->x * s) / MY_SCALE) * s;
		s = (glyph->outline.points+i)->y < 0 ? 1 : -1;
		(glyph->outline.points+i)->y = (((glyph->outline.points+i)->y * s) / MY_SCALE ) * s;
		//printf( "(%8d  %8d)\n", (glyph->outline.points+i)->x, (glyph->outline.points+i)->y );
	}
}
		   ...
		   const void *text, int cc, MWTEXTFLAGS flags)
{
	...
	ay += pf->fontsize*75/100;   // #### jsp
	//ay += pf->fontsize *68/100 + 4;   // #### jsp
	...
	if ((pf->fontrotation != 0)
		...
		FT_Render_Mode render_mode =
			(pf->
			 fontattr & MWTF_ANTIALIAS) ? FT_RENDER_MODE_NORMAL/*ft_render_mode_normal*/ :
		...
		for (i = 0; i < cc; i++) {
			...
			error = FT_Load_Glyph(face, curchar, FT_LOAD_NO_AUTOHINT | FT_LOAD_NO_SCALE );
		...
		for (i = 0; i < cc; i++) {
			...
			if ((blit_instructions.dstw > 0)
				...
				if (str[i] >= 0xe31 && str[i] < 0xe4e) {  /* check for Thai sub/super scripts */
					if (!( str[i]==0xe32 ||
					       str[i]==0xe33 ||
					      (str[i]>=0xe3b && str[i]<=0xe46) )) {
#warning test THAI
						...
						ax -= (face->glyph->advance.x >> 6);
#endif
						//blit_instructions.dstx -= sbit->width + 5;
						blit_instructions.dstx -= sbit->width + pf->fontsize/6;   // must scale

						//blit_instructions.dsty -= 2;
					}
				}
                           ...
                           MWCOORD * pbase)
{
	...
	for (char_index = 0; char_index < char_count; char_index++) {
		...
		if (str[char_index] >= 0xe31 && str[char_index] < 0xe4e) {  /* check for Thai sub/super scripts */
			if (!( str[char_index]==0xe32 ||
			       str[char_index]==0xe33 ||
			      (str[char_index]>=0xe3b && str[char_index]<=0xe46) )) {
				advance = 0;
			}
		}
