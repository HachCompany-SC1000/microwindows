	...
	MWCOORD height, char *path, int flags)
{
	...
	STATUS_TRACE(808000001);
	id = GdLoadImageFromFile(psd, path, flags);
	STATUS_TRACE(808000002);
	if (id) {
		STATUS_TRACE(808000003);
		GdDrawImageToFit(psd, x, y, width, height, id);
		STATUS_TRACE(808000004);
		GdFreeImage(id);
		STATUS_TRACE(808000005);
	}
}
...
GdLoadImageFromFile(PSD psd, char *path, int flags)
{
  ...
  STATUS_TRACE(808000006);
  fd = open(path, O_RDONLY);
  if (fd < 0 || fstat(fd, &s) < 0) {
    EPRINTF("GdLoadImageFromFile: can't open image: %s\n", path);
    return(0);
  }
  STATUS_TRACE(808000007);
#if HAVE_MMAP
  buffer = mmap(0, s.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
  STATUS_TRACE(808000008);
  if (!buffer) {
    STATUS_TRACE(808000009);
    EPRINTF("GdLoadImageFromFile: Couldn't map image %s\n", path);
    close(fd);
    return(0);
  }
#else
  buffer = calloc(s.st_size, 1);
  STATUS_TRACE(808000010);
  if (!buffer) {
     STATUS_TRACE(808000011);
  ...
  STATUS_TRACE(808000012);
  binit(&src, buffer, s.st_size);
  STATUS_TRACE(808000013);
  id = GdDecodeImage(psd, &src, path, flags);
  STATUS_TRACE(808000014);

#if HAVE_MMAP
  STATUS_TRACE(808000015);
  munmap(buffer, s.st_size);
#else
  STATUS_TRACE(808000016);
  free(buffer);
#endif

  STATUS_TRACE(808000017);
  close(fd);
  STATUS_TRACE(808000018);
  return(id);
}
#endif /* defined(HAVE_FILEIO) */
...
GdDecodeImage(PSD psd, buffer_t * src, char *path, int flags)
{
	...
	STATUS_TRACE(808000020);
	pimage = (PMWIMAGEHDR)malloc(sizeof(MWIMAGEHDR));
	STATUS_TRACE(808000021);
	if(!pimage) {
		return 0;
	}
	...
	if (path) {
		STATUS_TRACE(808000022);
		loadOK = LoadTIFF(path, pimage);
		STATUS_TRACE(808000023);
	}
#endif
#if defined(HAVE_JPEG_SUPPORT)
	if (loadOK == 0) {
		STATUS_TRACE(808000024);
		loadOK = LoadJPEG(src, pimage, psd, flags);
		STATUS_TRACE(808000025);
	}
#endif
#if defined(HAVE_BMP_SUPPORT)
	if (loadOK == 0)
	...
	if (loadOK == 0) {
		STATUS_TRACE(808000026);
		EPRINTF("GdDecodeImage: unknown image type\n");
		goto err;		/* image loading error*/
	}
	if (loadOK != 1) {
		STATUS_TRACE(808000027);
		goto err;		/* image loading error*/
	}

	/* allocate id*/
	STATUS_TRACE(808000028);
	pItem = GdItemNew(IMAGEITEM);
	STATUS_TRACE(808000029);
	STATUS_TRACE(pItem);
	...
	pItem->pimage = pimage;
	pItem->psd = psd;
	STATUS_TRACE(808000030);
	GdListAdd(&imagehead, &pItem->link);
	STATUS_TRACE(808000031);
	STATUS_TRACE(pItem->id);
	return pItem->id;

err:
	STATUS_TRACE(808000032);
	free(pimage);
	STATUS_TRACE(808000033);
	...
	int id)
{
	...
	if (!pItem)
		return;
	pimage = pItem->pimage;
	STATUS_TRACE(pimage->height);
	STATUS_TRACE(pimage->width);
	...
	if (height != pimage->height || width != pimage->width) {
		...
		STATUS_TRACE(808003004);
		GdDrawImage(psd, x, y, &image2);
		STATUS_TRACE(808003005);
		free(image2.imagebits);
		STATUS_TRACE(808003006);
	} else {
		STATUS_TRACE(808003007);
		GdDrawImage(psd, x, y, pimage);
	}
	STATUS_TRACE(808003008);
}



int GdLoadScaledImage( PSD psd, MWIMAGEHDR *dstIm, MWCLIPRECT *pDstRect, char *srcFile)
{
	int id;
	MWCLIPRECT srcRect, dstRect;
	IMAGEITEM *pImageItem;

	if (dstIm->imagebits) {
		free(dstIm->imagebits);
		dstIm->imagebits = NULL;
	}

	id = GdLoadImageFromFile( psd, srcFile, 0/*flags*/ );
	if (!id)
		return -1;

	pImageItem = findimage(id);
	if (!pImageItem)
		return -2;

	srcRect.x = srcRect.y = 0;
	srcRect.width = pImageItem->pimage->width;
	srcRect.height = pImageItem->pimage->height;

	dstRect.x = dstRect.y = 0;
	dstRect.width = pDstRect->width;
	dstRect.height = pDstRect->height;

	dstIm->width = pDstRect->width;
	dstIm->height = pDstRect->height;
	dstIm->planes = pImageItem->pimage->planes;
	dstIm->bpp = pImageItem->pimage->bpp;
	dstIm->compression = pImageItem->pimage->compression;
	dstIm->palsize = pImageItem->pimage->palsize;


	ComputePitch(pImageItem->pimage->bpp, pDstRect->width, &dstIm->pitch, &dstIm->bytesperpixel);

	dstIm->imagebits = (unsigned char*)malloc(dstIm->pitch * dstIm->height);

	GdStretchImage(pImageItem->pimage, &srcRect, dstIm, &dstRect);

	// we only keep the scaled imagebits, the image is not used anymore:
	GdFreeImage(id);
...
GdFreeImage(int id)
{
	...
	if (pItem) {
		...
		/* delete image bits*/
		if(pimage->imagebits) {
			STATUS_TRACE(808003011);
			STATUS_TRACE(pimage->imagebits);
			free(pimage->imagebits);
			STATUS_TRACE(808003012);
		}
		if(pimage->palette) {
			STATUS_TRACE(808003013);
			STATUS_TRACE(pimage->palette);
			free(pimage->palette);
			STATUS_TRACE(808003014);
		}

		STATUS_TRACE(808003015);
		STATUS_TRACE(pimage);
		free(pimage);
		STATUS_TRACE(808003016);
		STATUS_TRACE(pItem);
		GdItemFree(pItem);
		STATUS_TRACE(808003017);
	}
}
...
{
	long use_bytes;
	if (inptr->offset + num_bytes >= inptr->size)
		use_bytes =  inptr->size - inptr->offset;
	else
		use_bytes = num_bytes;
	cinfo->src->next_input_byte += use_bytes;
	cinfo->src->bytes_in_buffer -= use_bytes;
}
#endif
...
my_error_exit (j_common_ptr cinfo)
{
  ...
  STATUS_TRACE(808001000);
  STATUS_TRACE(cinfo->err->msg_code);
  (*cinfo->err->output_message) (cinfo);
  STATUS_TRACE(808001001);
...
LoadJPEG(buffer_t * src, PMWIMAGEHDR pimage, PSD psd, MWBOOL fast_grayscale)
{
	...
	STATUS_TRACE(808000100);
	bseek(src, 0, SEEK_SET);
	STATUS_TRACE(808000101);
	if (!bread(src, magic, 2)) {
		STATUS_TRACE(808000102);
		return 0;
	}
	if (magic[0] != 0xFF || magic[1] != 0xD8) {
		STATUS_TRACE(808000103);
		return 0;	/* not JPEG image */
	}

	bread(src, magic, 8);
	if ((strncmp((char*)magic+4, "JFIF", 4) != 0) &&
	    (strncmp((char*)magic+4, "Exif", 4) != 0) ) {
	    	STATUS_TRACE(808000104);
		return 0;	/* not JPEG image */
	}

	STATUS_TRACE(808000105);
	bseek(src, 0, SEEK_SET);
	pimage->imagebits = NULL;
	pimage->palette = NULL;
	...
	STATUS_TRACE(808000106);
	if (setjmp(jerr.setjmp_buffer)) {
    		/* If we get here, the JPEG code has signaled an error.
		 * We need to clean up the JPEG object, close the input file, and return.
		 */
		STATUS_TRACE(808000107);
		jpeg_destroy_decompress(&cinfo);
		STATUS_TRACE(808000108);
		return 0;
	}
	STATUS_TRACE(808000108);
...
fastjpeg:
	...
	STATUS_TRACE(808000115);
	jpeg_calc_output_dimensions(&cinfo);
	STATUS_TRACE(808000116);
	STATUS_TRACE(cinfo.output_width);
	STATUS_TRACE(cinfo.output_height);
	...
	STATUS_TRACE(808000117);
	STATUS_TRACE(pimage->pitch * pimage->height);
	STATUS_TRACE(pimage->palsize);
	...
	if(pimage->bpp <= 8) {
		STATUS_TRACE(808000118);
		STATUS_TRACE(256*sizeof(MWPALENTRY));
		pimage->palette = malloc(256*sizeof(MWPALENTRY));
		STATUS_TRACE(pimage->palette);
		if(!pimage->palette)
			goto err;
		if (fast_grayscale) {
			STATUS_TRACE(808000130);
			for (i=0; i<256; ++i) {
				MWPALENTRY pe;
	...
	STATUS_TRACE(808000119);
	/* Step 5: Start decompressor */
	jpeg_start_decompress (&cinfo);
	STATUS_TRACE(808000120);

	/* Step 6: while (scan lines remain to be read) */
	while(cinfo.output_scanline < cinfo.output_height) {
		/* catch constant scanline bug: */
		if (last_scanline!=-1 && last_scanline == cinfo.output_scanline) {
			STATUS_TRACE(808000300);
			ret = 0;
			// goto err:
			exit (2); /* stop to catch the error */
		}
		...
		STATUS_TRACE(808000121);
		STATUS_TRACE(cinfo.output_scanline);
		jpeg_read_scanlines (&cinfo, rowptr, 1);
	}
	STATUS_TRACE(808000122);
	ret = 1;

err:
	/* Step 7: Finish decompression */
	STATUS_TRACE(808000123);
	STATUS_TRACE(cinfo.src->bytes_in_buffer);
	jpeg_finish_decompress (&cinfo);
	STATUS_TRACE(808000124);

	/* Step 8: Release JPEG decompression object */
	jpeg_destroy_decompress (&cinfo);
	STATUS_TRACE(808000125);
