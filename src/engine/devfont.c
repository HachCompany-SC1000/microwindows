 ...
 */
extern int pixel_width_hook_is, pixel_width_hook_shall;
void GdCreateFontSetWidthHook(int is, int shall)
{
	pixel_width_hook_is = is;
	pixel_width_hook_shall = shall;
	return;
}
PMWFONT
	...
	if (fontclass == MWLF_CLASS_ANY || fontclass == MWLF_CLASS_PCF) {
 	...
 	if (fontclass == MWLF_CLASS_ANY || fontclass == MWLF_CLASS_FREETYPE) {
		if (freetype2_init(psd)) {
			static int s_useAntiAlias = 0;  // 0: read cfg, 1: no, 2: yes
			/* FIXME auto antialias for height > 14 for kaffe*/
#if 0
			if (plogfont && plogfont->lfHeight > 14 &&
				plogfont->lfQuality)
					fontattr |= MWTF_ANTIALIAS;
#else
			if (!s_useAntiAlias) {  // read antialias - cfg from file:
				int fh = open("/mnt/sc1000/set/antialias", O_RDONLY);
				char bf[4] = {0};
				if (fh < 0) {
					s_useAntiAlias = 2;
				}
			...
			fontattr |= (s_useAntiAlias==2 ? MWTF_ANTIALIAS : 0); // jsp
			//fontattr |= MWTF_KERNING;  // jsp
#endif
